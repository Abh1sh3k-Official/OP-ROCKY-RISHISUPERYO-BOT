...
HELP_STRINGS = f"""
`YO.. MAI` OP ROCKY RISHISUPERYO BOT[⚡](https://telegra.ph/file/565caa7476ff1f50fcc7e.jpg)
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(
        "Rishisuperyo.modules." + module_name)
...
def send_start(update, context):
    ...
    text = PM_START_TEXT
    buttons = [[InlineKeyboardButton(text="➵ꗃℍ𝔼𝕃ℙ 🇦 🇳 🇩  C͛M͛D͛S͛ ∞ϟ",
                                  callback_data="help_back"),
    ]]
             

    buttons += [[InlineKeyboardButton(text="Close the Menu ツ✪",
                                  callback_data="close_menu")]]
...
def get_help(update, context):
    ...
    if chat.type != chat.PRIVATE:
        ...
        update.effective_message.reply_text(
            "PM MA AA CMD LIST PANE KE LIYE VRO🙂",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
...
def get_settings(update, context):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            text="Yaha click kar settings ke liye😎😏"
            msg.reply_text(
                text,
                reply_markup=InlineKeyboardMarkup(
                    [
...
def main():
    ...
    if WEBHOOK:
        ...
        updater.bot.send_message(
            ...
            text="Who do you think you are, Really God😂")
        client.run_until_disconnected()
